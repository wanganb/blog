...
$.get("./content/index.json",function(result){
    if(result.item_list){
        ...
        var tagNames={"tech":"技术","bytalk":"杂谈","essay":"随笔","share":"分享"};
        for(var i=0;i<items.length;i++){
            ...
            '<div class="item_list_tag"><a href="#'+items[i].tag+'"><i class="fa fa-tag" aria-hidden="true"></i> Tag : '+tagNames[items[i].tag]+'</a> ; <a href="#All"><i class="fa fa-tag" aria-hidden="true"></i> Tag : 所有分类</a></div>'+
            '</div></div>');
        }
    }
    AddFilter();
